...
public class MancalaDataStructure implements Serializable{
    ...
    private int finalPos; // final position of stone after distributeStones()
    ...
    public void emptyStores() {
        Countable storeOne = data.get(storePos(1));
        storeOne.removeStones();

        Countable storeTwo = data.get(storePos(2));
        storeTwo.removeStones();
    }
    ...
    public void setUpPits() {
        // Empty pits before setting them up
        for (int i = 0; i < PLAYER_ONE; i++) {
